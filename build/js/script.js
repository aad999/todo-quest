...
function show_hero_image() {
    document.querySelector(".hero-image").setAttribute("src", `./assets/${heroes[selection].name}.webp`);
...
function show_cards() {
    ...
    heroes.forEach((hero, index) => {
        ...
        if(index == 0){
            ...
            <img src="./assets/${name}.webp" alt="" srcset="" index="${index}" class="card-image ${col3}" onclick=buy(${index})>
            </div>`;
            ...
            let to_insert = `<div class="card">
            <img src="./assets/locked.png" alt="" srcset="" index="${index}" class="card-image ${col3}" onclick=buy(${index})>
            </div>`;
            document.querySelector(".blank-end").insertAdjacentHTML("beforebegin", to_insert);
        }
        else if (!bought) {
            ...
            let to_insert = `<div class="card">
            <img src="./assets/${name}_unbought.png" alt="" srcset="" index="${index}" class="card-image ${col3}" onclick=buy(${index})>
            </div>`;
            document.querySelector(".blank-end").insertAdjacentHTML("beforebegin", to_insert);
        }
    });
...
function show_tasks() {
    ...
    tasks.forEach((task, index) => {
        let to_insert = `<div class="pushed">
            ...
            <button index="${index}" title="push" type="submit" class="push delete bg-amber-500/50 border-2 border-amber-500 shadow-sm hover:shadow-amber-400" onclick=delete_task(this)><img src="./assets/delete.svg" alt="" srcset="" class="icon"></button>
            <button index="${index}" title="push" type="submit" class="push done bg-green-500/50 border-2 border-green-500 shadow-sm hover:shadow-green-400" onclick=done_task(this)><img src="./assets/done.svg" alt="" srcset="" class="icon"></button>
        </div>`
